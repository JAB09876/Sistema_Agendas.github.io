...
class medicoModel
{
    ...
    public function all()
    {
        try {
            // Consulta SQL
            $vSQL = "SELECT m.id, m.nombre, m.idEspecilidad, m.idSucursal,
                                e.descripcion AS especialidad, 
                                s.nombre AS nombre_sucursal
                            FROM Medico m
                            JOIN Especialidad e ON m.idEspecilidad = e.id
                            JOIN Sucursal s ON m.idSucursal = s.id
                            WHERE m.estado = 1;";
    ...
    public function get($id)
    {
        try {
            // Consulta SQL
            $vSQL = "SELECT m.id, m.nombre AS nombre_medico, m.estado,
                    e.descripcion AS especialidad, 
                    s.nombre AS nombre_sucursal
                FROM Medico m
                JOIN Especialidad e ON m.idEspecilidad = e.id
                JOIN Sucursal s ON m.idSucursal = s.id
                WHERE m.estado = 1 AND m.id = $id;";
            // Ejecutar la consulta
            $vResultado = $this->enlace->executeSQL($vSQL);
